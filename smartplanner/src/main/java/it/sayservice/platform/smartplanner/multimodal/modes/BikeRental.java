...
public class BikeRental {
	...
	/** phase two output bikestation->destination. **/
	 ...
	 * @param itnHelper
	 */
	public BikeRental(OTPConnector otpCon, ItineraryBuildHelper itnHelper, RepositoryUtils repoUtils) {
	...
	public List<Itinerary> getItinerary(String router, HashMap<String, Object> parameters) throws IOException {
		...
		/**
		 * search a bike rental station near to source using geo spatial
		 * calculation after validation(w.r.t to rental bikes). check if the
		 ...
		 * station. | |- get a bicycle plan from station to destination `- merge
		 * the two plans to create one itinerary as output.
		 */
		...
		if (parameters.get(Constants.SP_RQ_FROM) != null) {
			...
			if (from != null) {
				if (!from.getLat().isEmpty() && !from.getLon().isEmpty()) {
					...
					bikeStations = repositoryUtils.findBikeRentalStationPositionByNear(router, new Point(sX, sY), true, parameters);
					Collections.sort(bikeStations, new BikeStationComparator(helper, sX, sY));
		...
		if ((sX != 0 & sY != 0) && (dX != 0 & dY != 0)) {
			...
			if (bikeStations.size() > 0) {
				BikeStation bs = bikeStations.get(0);
				/** perform analysis **/
				...
				if (distToDestination < distToStation) {
					...
					/** get rental plan. **/
					// create map for otp invocation invoke OTP and get List
					// Itinerary step1 default values for OTP map
					otpMap.put(Constants.OTP_RQ_DATE, date);
					otpMap.put(Constants.OTP_RQ_TIME, time);
					otpMap.put(Constants.OTP_RQ_ARRIVEBY, arriveBy);
					otpMap.put(Constants.OTP_RQ_OPTIMIZE, optimize);
					...
					otpMap.put(Constants.OTP_RQ_MODE, parameters.containsKey("extraTransport")
							? (String) parameters.get("extraTransport") : Constants.MODES.TRANSIT.toString());
					otpMap.put(Constants.OTP_RQ_FROM, source);
					otpMap.put(Constants.OTP_RQ_TO, bs.getPosition()[0] + "," + bs.getPosition()[1]);
					// connect and fetch data.
					String responseOne = otpConnector.connect(router, otpMap);
					// takes station and mode information to be used later.
					HashMap<String, Object> preProcessParams = (HashMap<String, Object>) parameters.clone();
					preProcessParams.put(Constants.SP_RQ_FROMSTATION, bs);
					preProcessParams.put(Constants.SP_RQ_USERMODE, TType.SHAREDBIKE_WITHOUT_STATION.name());
					preProcessParams.put(Constants.ROUTER, router);
					// process data to smart planner format
					...
					otpMap.put(Constants.OTP_RQ_FROM, bs.getPosition()[0] + "," + bs.getPosition()[1]);
					otpMap.put(Constants.OTP_RQ_TO, destination);
					// connect and fetch data.
	...
	private List<Leg> getRecurrLegsWithinPeriod(String router, HashMap<String, Object> parameters, Long interval) throws IOException {
		...
		try {
			...
			String requestedTime = localParams.get(Constants.SP_RQ_DATE) + " "
					+ localParams.get(Constants.SP_RQ_DEPTIME);
			...
			while (search) {
				// get itinerary with time = starting time and number of itns =
				// 1.
				List<Itinerary> searchItn = getItinerary(router, localParams);
				// add itn to return list
				tempList.addAll(searchItn);
				// check if the last itn endTime is less than mustReachTime.
				if (searchItn.isEmpty() || searchItn.size() < 1) {
					...
					localParams.put(Constants.SP_RQ_DATE, dateFormatter.format(strDate));
